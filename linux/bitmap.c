...
/**
 * bitmap_parse_user - convert an ASCII hex string in a user buffer into a
 * bitmap
 *
...
int bitmap_parse_user(const char __user *ubuf, unsigned int ulen,
    ...
    if (!access_ok(VERIFY_READ, ubuf, ulen))
        return -EFAULT;
    return __bitmap_parse((const char __force *)ubuf, ulen, 1, maskp,
                          nmaskbits);
}
...
int bitmap_parselist_user(const char __user *ubuf, unsigned int ulen,
    ...
    if (!access_ok(VERIFY_READ, ubuf, ulen))
        return -EFAULT;
    return __bitmap_parselist((const char __force *)ubuf, ulen, 1, maskp,
                              nmaskbits);
}
...
void bitmap_fold(unsigned long *dst, const unsigned long *orig, unsigned int sz,
    ...
    for_each_set_bit(oldbit, orig, nbits) set_bit(oldbit % sz, dst);
}
...
unsigned int bitmap_from_u32array(unsigned long *bitmap, unsigned int nbits,
    ...
    for (src_idx = dst_idx = 0; dst_idx < BITS_TO_LONGS(nbits); ++dst_idx) {
        ...
        if (dst_idx < nbits / BITS_PER_LONG)
            ...
            bitmap[dst_idx] = (bitmap[dst_idx] & ~mask) | (part & mask);
        }
    }
 ...
 */
#ifdef __BIG_ENDIAN
void bitmap_copy_le(unsigned long *dst, const unsigned long *src,
                    unsigned int nbits) {
    unsigned int i;
